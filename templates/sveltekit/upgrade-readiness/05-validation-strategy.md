...
rm -f pnpm-lock.yaml package-lock.json yarn.lock   # remove the repo's lockfile
<pm> install
<pm> install 2>&1 | grep -i "peer\|warn\|error"
<pm> list --depth 0 | grep -i "sveltejs\|svelte\|vite"
```
...
Start the app locally (via adapter-node, `<pm> preview`, or deployed preview URL):
...
- [ ] `<pm> install` — zero peer errors
- [ ] `<pm> exec svelte-kit sync` — exit 0
- [ ] `<pm> build` — exit 0
