...
This impact table is the **direct input to a `migration` execution plan**: each row's
before→after pattern becomes a uniform transform-rule mapping, its count seeds the checklist rows,
and its effort/severity classes drive sequencing and priority. Do not execute the rewrites here —
hand this table to the `migration` plan (or `refactor`, for the internal-restructure portion).
