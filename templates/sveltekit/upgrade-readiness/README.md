...
- Out: feature code rewrites — these are executed with a **`migration`** plan (external-driven
  code changes: repoint every breaking-change site to the new API, then retire the old surface) or a
  **`refactor`** plan (internal-driven restructures). This template produces the readiness inputs they
  consume; see Phase 3.

## Where this fits

`upgrade-readiness` is the **pre-flight / go-no-go stage** of the migration family. It audits and
quantifies the change, then hands off:

- Phase 3's breaking-change impact table (before→after pattern, detection, effort, count) is the
  direct input to a **`migration`** execution plan's mapping.
- If the change also entails internal restructuring, that portion is executed via **`refactor`**.

Run `upgrade-readiness` first; only after its go/no-go do you open the execution plan(s).
