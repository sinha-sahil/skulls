...
First, **detect the package manager** — the lockfile and `packageManager` field decide it. Every
command in this template that would run a package manager is written as `<pm>`; substitute the
detected tool (`pnpm` / `npm` / `yarn`) throughout the plan.

```bash
# Detect <pm>: pnpm-lock.yaml -> pnpm, package-lock.json -> npm, yarn.lock -> yarn
...
- [ ] `<pm> install --dry-run` passes (or conflicts documented)
